...
void test_prompt_encoding(
    Tokenizer &tokenizer, std::string prompt, int *expected_tokens, int num_expected_tokens
) {
    // encode
...
void test_prompt_encodings() {
    ...
    //     tokenizer, prompt0, expected_tokens0, sizeof(expected_tokens0) / sizeof(int)
    // );
    ...
    // test 1
    std::string prompt = "I believe the meaning of life is";
    ...
    // test 2
    std::string prompt2 = "Simply put, the theory of relativity states that ";
    int expected_tokens2[] = {
        1, 3439, 17632, 1925, 29892, 278, 6368, 310, 14215, 537, 5922, 393, 29871
    };
    test_prompt_encoding(
        tokenizer, prompt2, expected_tokens2, sizeof(expected_tokens2) / sizeof(int)
    );

    // test 3
    std::string prompt3 = "A brief message congratulating the team on the launch:\n\n        Hi "
                          "everyone,\n\n        I just ";
    int expected_tokens3[] = {1,     319,   11473, 2643, 378,   629, 271, 18099, 278,
                              3815,  373,   278,   6826, 29901, 13,  13,  4706,  6324,
                              14332, 29892, 13,    13,   4706,  306, 925, 29871};
    test_prompt_encoding(
        tokenizer, prompt3, expected_tokens3, sizeof(expected_tokens3) / sizeof(int)
    );
    ...
    int expected_tokens4[] = {1,     4103, 9632, 4223, 304,   5176, 29901, 13,   13,   4706,
                              7205,  4932, 357,  1149, 301,   449,  276,   316,  2778, 13,
                              4706,  1236, 407,  837,  524,   1149, 6042,  354,  772,  440,
                              29878, 1318, 13,   4706, 715,   1878, 330,   3055, 1725, 1149,
                              330,   3055, 1725, 4639, 28754, 13,   4706,  923,  968,  1149};
    test_prompt_encoding(
        tokenizer, prompt4, expected_tokens4, sizeof(expected_tokens4) / sizeof(int)
    );
}
